      ...
      >
        {processSteps.map((step, i) => (
          <motion.li key={step.title} variants={itemVariants} className="relative">
            <div className="h-full rounded-2xl border border-ink-200 bg-white p-7 shadow-soft transition-all duration-300 hover:-translate-y-1.5 hover:shadow-lift">
            ...
            {i < processSteps.length - 1 && (
              <div className="absolute top-1/2 -right-3 hidden h-px w-6 bg-ink-200 md:block" />
            )}
          </motion.li>
        ))}
      </motion.ol>
